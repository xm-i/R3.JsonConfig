...
public class GeneratorSpecTest {
	...
	/// <summary>
	/// [GenerateJsonConfigDto] がないクラスから DTO が生成されないことを検証する。
	/// </summary>
	[Fact]
	public async Task NoDtoGenerated_WhenAttributeIsMissing() {
		...
		runResult.Results.SelectMany(r => r.GeneratedSources).ShouldBeEmpty(
			"[GenerateJsonConfigDto] がないクラスからは DTO が生成されてはいけない");
	}

	/// <summary>
	/// struct に [GenerateJsonConfigDto] を付与してもジェネレータに無視されることを検証する。
	...
	/// <summary>
	/// record に [GenerateJsonConfigDto] を付与してもジェネレータに無視されることを検証する。
	...
	/// <summary>
	/// [GenerateJsonConfigDto] が付与されたクラスから DTO が生成され、コンパイルエラーが発生しないことを検証する。
	/// </summary>
	[Fact]
	public async Task DtoGenerated_WhenClassHasGenerateAttribute() {
		...
		runResult.Results.SelectMany(r => r.GeneratedSources).ShouldNotBeEmpty(
			"[GenerateJsonConfigDto] 付きクラスから DTO が生成される必要がある");
	}
	...
	/// <summary>
	/// [GenerateJsonConfigDto] を持つ型を通常プロパティとして保持する場合、
	...
	public async Task NestedModelWithAttribute_IsMappedTo_ForJsonType_AsPlainProperty() {
		...
		parentCode.Contains("global::TestNamespace.ChildForJson? ChildProp").ShouldBeTrue(
			"ネストされた [GenerateJsonConfigDto] 付きモデルは ForJson 型に変換されるべき");
	}

	/// <summary>
	/// [GenerateJsonConfigDto] を持つ型を ReactiveProperty&lt;T&gt; で保持する場合、
	...
	/// <summary>
	/// [GenerateJsonConfigDto] を持つ型を ObservableList&lt;T&gt; で保持する場合、
	/// DTO 側では TForJson[]? にマッピングされることを検証する。
	/// </summary>
	[Fact]
	...
	/// <summary>
	/// [GenerateJsonConfigDto] を持たないネスト型は ForJson 化されず、
	...
	public async Task NestedModelWithoutAttribute_IsKeptAsOriginalType() {
		...
		// [GenerateJsonConfigDto] がない型は ForJson 化されず元の型のまま
	...
	public async Task CreateModel_HasCorrectSignature() {
		...
		code.Contains("public static global::TestNamespace.MyModel? CreateModel(global::TestNamespace.MyModelForJson? json, global::System.IServiceProvider sp, global::GenJsonConfig.ReferenceResolver? resolver = null)").ShouldBeTrue(
	...
	public async Task CreateJson_HasCorrectSignature() {
		...
		code.Contains("public static global::TestNamespace.MyModelForJson? CreateJson(global::TestNamespace.MyModel? model, global::GenJsonConfig.ReferenceTracker? tracker = null)").ShouldBeTrue(
	...
	public async Task CreateModel_AssignsToValue_ForReactiveProperty() {
		...
		code.ShouldContain(".Set(model.Title, e);");
	}
	...
	public async Task CreateJson_ExtractsValue_ForReactiveProperty() {
		...
		code.ShouldContain(".Get(model.Title)");
	}
	...
	/// <summary>
	/// [GenerateJsonConfigDto] を持たないカスタム型を ReactiveProperty&lt;T?&gt; で保持する場合、
	...
	public async Task CustomType_WithoutAttribute_IsMappedTo_NullableT_AsReactiveProperty() {
		...
		code.Contains("HexColorForJson").ShouldBeFalse(
			"[GenerateJsonConfigDto] がない型は ForJson 化されてはいけない");
	}

	/// <summary>
	/// [GenerateJsonConfigDto] を持たないカスタム型を ObservableList&lt;T?&gt; で保持する場合、
	/// DTO 側では T?[]? のままマッピングされることを検証する（ForJson 化されない）。
	/// Color など JsonConverter で扱う型はこのルートを通る。
	/// </summary>
	[Fact]
	...
	/// <summary>
	/// [GenerateJsonConfigDto] を持たないカスタム型を通常プロパティとして保持する場合、
	/// DTO 側では T? のままマッピングされることを検証する（ForJson 化されない）。
	/// </summary>
	[Fact]
	...
	public async Task CreateModel_AssignsToValue_ForReactivePropertyOfCustomNullableType() {
		...
		code.ShouldContain(".Set(model.ColorRp, e);");
	}
	...
	public async Task CreateJson_ExtractsValue_ForReactivePropertyOfCustomNullableType() {
		...
		code.ShouldContain(".Get(model.ColorRp)");
	}
	...
	public async Task PolymorphicBase_GeneratesDtoWithStjAttributes() {
		...
		code.ShouldContain("global::GenJsonConfig.ForJsonConverterRegistry.CreateJson<global::TestNamespace.IBase, global::TestNamespace.IBaseForJson>");
	}
	...
	public async Task PolymorphicBase_GeneratesDispatchLogic() {
		...
		code.ShouldContain("global::GenJsonConfig.ForJsonConverterRegistry.Register<");
	}

	/// <summary>
	/// プロパティの型が [GenerateJsonConfigDto] を持つインターフェースである場合、
	...
	public async Task ReactivePropertyOfPolymorphicInterface_IsMappedToInterfaceDto() {
		...
		// CreateModel のロジック
		code.ShouldContain(".Set(model.BaseRp, global::TestNamespace.IBaseForJson.CreateModel(e, sp, resolver));");

		// CreateJson のロジック
		code.ShouldContain(".CreateJson(new global::ReactivePropertyAdapter<global::TestNamespace.IBase>().Get(model.BaseRp), tracker),");
	}
	...
	/// [GenerateJsonConfigDto] 付きのネスト型を型引数として持つ場合、
	...
	public async Task CustomWrapper_OfNestedDtoType_IsMappedToForJsonType() {
		...
		// Child は [GenerateJsonConfigDto] 付きなので MyBox<Child> は ChildForJson? にマッピングされるべき
