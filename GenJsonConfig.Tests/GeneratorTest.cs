...
""";
	...
	/// <summary>
	/// GenerateJsonConfigDto属性が付与されていないクラスの場合、ソースコードが生成されないことを検証します。
	/// </summary>
	[Fact]
...
""";
	...
	/// <summary>
	/// GenerateJsonConfigDtoが付与されたネストされたクラスのプロパティ（ReactivePropertyおよびObservableListを含む）が
	/// 正しくDTOの型（末尾にForJsonが付く型）に変換されることを検証します。
	/// </summary>
	[Fact]
	public async Task ShouldGenerateNestedDtoTypes_WhenNestedClassesHaveGenerateJsonConfigDtoAttribute() {
		// Arrange
		var source = """
using R3;
using GenJsonConfig.Attributes;
...
""";
		...
		generatedSource.Contains("public static global::TestNamespace.MethodCheckConfig? CreateModel(global::TestNamespace.MethodCheckConfigForJson? json, global::System.IServiceProvider sp, global::GenJsonConfig.ReferenceResolver? resolver = null)").ShouldBeTrue("CreateModelメソッドが生成される必要があります");

		// CreateJson メソッドの存在とシグネチャ検証
		generatedSource.Contains("public static global::TestNamespace.MethodCheckConfigForJson? CreateJson(global::TestNamespace.MethodCheckConfig? model, global::GenJsonConfig.ReferenceTracker? tracker = null)").ShouldBeTrue("CreateJsonメソッドが生成される必要があります");
